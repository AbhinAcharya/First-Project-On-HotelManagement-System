...
if (isset($_GET['editevent'])) {
	//$con=mysqli_connect("localhost","root","","sirledinuvako");
	$id = $_GET['editevent'];
	$rec = mysqli_query($con,"SELECT * FROM specialevent WHERE ID=$id");
	$record = mysqli_fetch_array($rec);
...
<?php endif?>
	...
	<?php else: ?>
		...
		<?php endif ?>
		<br>Note:Insert < br > to move to next line
	</form>
...
{?>
	...
	<tr><td><?php echo $row['id']?></td>
		...
		<td><a href="displayanddelete.php?editevent=<?php echo $row['id'];?>">edit</a></td> 
		<td><a href="../database/backenddatabase.php?delevent=<?php echo $row['id'];?>" >delete</a></td>
...
<h4 class="tbl"><div class="test">If you want to insert the data in table then plz fill the form </div></h4>
	<form method="post" action="../database/backenddatabase.php" enctype="multipart/form-data" class="tbl" name="specialevent">
		<input type="hidden" name="ID" value="<?php echo $id; ?>">
		<label>EventName:</label>
		<input type="Text" name="eventname" value="<?php echo $eventname;?>"   required="required"><br>
		<label>Location</label>
		<input type="Text" name="location" value="<?php echo $location;?>"   required="required"><br>
		<label>Description:</label>
		<input type="text" name="description" value="<?php echo $description;?>" required="required"><br>
		<label>Image:</label><img src="../img_src/<?php echo $imagename?>" class="img"><input type="file" name="upfile"><br/>
		<!-- <label>Password:</label>
		...
		<!-- <label>Address:</label>
		<input type="text" name="address" value="<?php echo $address;?>" required="required"><br> -->
		<?php if ($edit_state_event === false): ?>
			
		<input type="submit" name="insertevent" value="insert" class="insert">
		<input type="reset" name="reset" class="reset">
	<?php else: ?>
		<input type="submit" name="updateevent" value="update" class="update">
